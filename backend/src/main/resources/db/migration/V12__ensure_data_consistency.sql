-- Ensure admin user exists (generate a new hash with HashMaker.java if changing password)
INSERT INTO users (name, email, password_hash, role, max_hours_per_week, typical_hours_per_week)
SELECT 'Admin User', 'admin@YOUR_DOMAIN_HERE', '$2a$10$PYFp4vF5/izccFo.m5f7YuHQVMfH/iYKw8gr2HGb/nzAVhjb9C3Oy', 'ADMIN', 40, 21
WHERE NOT EXISTS (SELECT 1 FROM users WHERE email = 'admin@YOUR_DOMAIN_HERE');
